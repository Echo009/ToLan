(function (window) {
    ...
    function inheart(x, y, r) {  // is this point  inheart ？ 
        // x^2+(y-(x^2)^(1/3))^2 = 1
    ...
    Heart = function () {
        ...
        for (var i = 10; i < 30; i += 0.2) {
            ...
            y = 14 * Math.cos(t) - 6 * Math.cos(2 * t) - 2.2 * Math.cos(3 * t) - Math.cos(4 * t); // adjust the coefficient
    ...
    Seed.prototype = {
        ...
        hover: function (x, y) {  // the hover animation of bloom 
    ...
    Footer.prototype = {
        draw: function () {
            ...
            ctx.save();
            ctx.strokeStyle = '#434343';//the color of footer
    ...
    Tree.prototype = {
        ...
        jump: function () {
            ...
            if ((blooms.length && blooms.length < 4) || !blooms.length) { // the animate of flower fall 
                var bloom = this.opt.bloom || {},
                        width = bloom.width || this.width,
                        height = bloom.height || this.height,
                        figure = this.seed.heart.figure;
                ...
                for (var i = 0; i < random(1, 3); i++) {
                    blooms.push(this.createBloom(width / 2 + width, height, r, figure, null, 1, null, 1, new Point(random(-100, 600), 720), random(200, 300)));//new Point(random(-100,600), 720), random(200,300)
                }
            }
        }
    },
    ...
    Branch.prototype = {
        grow: function () {
            var s = this, p;
            if (s.len <= s.length) {
                p = bezier([s.point1, s.point2, s.point3], s.len * s.t* 1.00719);
                s.draw(p);
                s.len += 1;
                s.radius *= 0.971; //
            } else {
        ...
        draw: function (p) {
            ...
            ctx.save();
            ctx.beginPath();
            ctx.fillStyle = '#434343';  // the color of branch 
            ctx.shadowColor = '#0C0C0C';//the color of branch shadow    rgb(35, 31, 32)
            ctx.shadowBlur = 2;
            ctx.moveTo(p.x, p.y);
    ...
    Bloom = function (tree, point, figure, color, alpha, angle, scale, place, speed) {  // main object 
        this.tree = tree;
        this.point = point;
        this.color = color || 'rgb(255,' + random(20, 192) + ',' + random(133, 203) + ')'; //  color  -> pink 
        this.alpha = alpha || random(0.1719, 1);
        this.angle = angle || (random(0, 360)+random(0, 360))/2;
        this.scale = scale || 0.100719; // the size of bloom 
        this.place = place;
        this.speed = speed;
    ...
    Bloom.prototype = {
        ...
        jump: function () {
            ...
            if (s.point.x < -20 || s.point.y > height + 20) {
                ...
                s.angle += 0.06719; // control the speed of rotate 
                s.speed -= 1.0719;
            }
        }
    },
